...
create unique index if not exists unit_hours_unit_weekday_unique
on public.unit_hours (unit_id, weekday);
...
alter table public.unit_hours enable row level security;
...
drop policy if exists unit_hours_read_authenticated on public.unit_hours;
create policy unit_hours_read_authenticated
on public.unit_hours
for select
to authenticated
using (true);

drop policy if exists unit_hours_admin_write on public.unit_hours;
create policy unit_hours_admin_write
on public.unit_hours
for all
to authenticated
using (
  public.is_admin()
  or public.is_admin_or_manager_of(unit_id)
)
with check (
  public.is_admin()
  or public.is_admin_or_manager_of(unit_id)
